...
class PropuestasController extends Controller
{
 ...
 /*
  * Metodo que obtiene el listado de propuestas de un cliente en particular
  */
  public function listadoPropuestas( $clienteID ) {
    $datos                 = array();
    $datos[ 'propuestas' ] = array();
    $propuestas = Propuestas::where( [ 'status' => 1 , 'clienteID' => $clienteID ]  )
  ...
  */
  public function datosPropuesta( $propuestaID , $modo = '' , $esEnvio=false) {
    ...
    $productos = PropuestasDetalle::where( 'idPropuesta' , $propuesta->id )->where( 'status' , '1' )->get();
  ...
  */
  public function statusPago( $propuestaID ) {
    $monto  = 0;
    $pagos = Pagos::where( 'propuestaID' , $propuestaID )->where( 'status' , 1 )->get();
