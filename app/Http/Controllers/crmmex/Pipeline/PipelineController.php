...
class PipelineController extends Controller
{
  ...
  public function actualizaIndicador( Request $request ) {
    ...
    if( $indicador->save() ) {
      ...
      DB::table( 'crmmex_pipeline_fases' )->where( 'indicadorID' , $request->indicadorID )
        ->whereNotIn( 'id' , $indices )->update( [ 'status' => 0 ] );
      ...
      DB::table( 'crmmex_pipeline_detalleindicador' )->where( 'indicadorID' , $request->indicadorID )
        ->whereNotIn( 'id' , $indicesD )->update( [ 'status' => 0 ] );
    }
  }
  ...
  public function detalleIndicador( $indicadorID ) {
    ...
    $fases = Fases::where( 'indicadorID' , $ind->id )->where( 'status' , 1 )->get();
    foreach( $fases AS $fase ) {
      $detalles = Detalles::where( 'faseID' , $fase->id )->where( 'indicadorID' , $ind->id )->where( 'status' , 1 )->get();
      foreach( $detalles AS $detalle ) {
        $datos[ 'detalle' ][] = array (
          'faseID'  => $fase->id,
          'idty'    => $detalle->id,
  ...
  private function faseID( $nombreFase , $indicadorID ) {
    $fase = Fases::where( 'nombreFase' , $nombreFase )->where( 'indicadorID' , $indicadorID )->first();
    return $fase->id;
  }
