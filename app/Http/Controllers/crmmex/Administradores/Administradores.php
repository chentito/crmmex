...
use App\Models\crmmex\Sistema\Imgejecutivo AS Img;
use Hash;
...
class Administradores extends Controller
{
  ...
  public function shortUpdate( Request $request ) {
    ...
    if( $request->hasFile( 'perfilLogo' ) ) {
      Log::warning("entraaa");
      $img = Img::updateOrCreate(
              [ 'ejecutivoID' => Auth::user()->id ] ,
              [ 'img'  => file_get_contents( $request->file( 'perfilLogo' )->getRealPath() ) , 'mime' => $request->file( 'perfilLogo' )->getMimeType() ]
            );
    } else {
      Log::warning("no entraaa");
    }
  ...
  public function imagenPerfil( $ejecutivoID ) {
    $img  = Img::where( 'ejecutivoID' , $ejecutivoID )->first();
    if( !$img ) {
      $img  = Img::where( 'ejecutivoID' , '0' )->first();
    }
    return response( $img->img )->header( 'Content-type' , $img->mime );
  }
