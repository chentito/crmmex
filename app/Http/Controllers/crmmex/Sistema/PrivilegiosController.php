...
class PrivilegiosController extends Controller
{
    ...
    public function listadoPrivilegios( $perfilID ) {
          $privilegios = Privilegios::where( 'status' , 1 )
                                    ->where( 'idRol' , $perfilID )
                                    ->get();
    ...
    public function guardaPrivilegios( Request $request ) {
        ...
        $secciones  = Secciones::where( 'status' , 1 )->get();

        foreach( $secciones AS $seccion ) {
            $privilegio = Privilegios::where( 'idRol' , $perfilID )->where( 'idSeccion' , $seccion->id )->first();

           if( $privilegio ) {
                Privilegios::where( 'idRol' , $perfilID )->where( 'idSeccion' , $seccion->id )
                           ->update( [ 'status' => ( ( !$request->has( 'seccion_' . $seccion->id ) ) ? 0 : 1 ) ] );
           } else {
