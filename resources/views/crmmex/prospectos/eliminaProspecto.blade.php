...
<div class="row mt-3">
  <div class="col-sm-12 text-center">
    <button class="btn btn-sm {{$btn}}" id="btnRegresaListadoProspectos">Regresar al listado</button>
  </div>
</div>
...
<script>
  ...
  document.getElementById( 'btnRegresaListadoProspectos' ).addEventListener( 'click' , function( e ) {
    e.preventDefault();
    regresarListadoProspectos();
  });
  ...
  function regresarListadoProspectos() {
  ...
  function eliminaCliente( mov ) {
      ...
      axios.post( url , {} , config )
           .then( response => {
             aviso( 'Prospecto ' + ( ( mov==2 ) ? 'deshabilitado' : 'eliminado' ) + ' correctamente' );
             contenidos( 'prospectos_listado' );
           })
           .catch( err => {
             console.log( err );
